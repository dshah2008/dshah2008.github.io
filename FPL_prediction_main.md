...
\
With this project, over multiple cycles, I plan to develop state-of-the-art models that can accurately forecast player performance multiple timesteps into the future. In this first version, I have simplified the problem to predict the average of the points scored in the next five games. The prediction also only considers appearances that accumulate between 0 and 50 points. The 0 lower limit is applied because from a business standpoint, we are not interested in predicting whether a player is going make an appearance or not. This is mostly known based on preferred starting line-ups or injuries. The upper limit of 50 is applied only in this iteration to simplify the problem.\
\
